// Copyright [2020] <João Vitor Maia Neves Cordeiro>
#ifndef NO_RB_HPP
#define NO_RB_HPP
...
class NoRB {
 private:
    //! Atributo onde é guardado o valor representado pelo nó
    T* dado;
    //! Atributo que aponta para menor nó descendente
    NoRB<T>* esquerda;  
    //! Atributo que aponta para maior nó descendente
    NoRB<T>* direita;  
    //! Atributo que aponta para nó ascendente
    NoRB<T>* pai;  
    //! Atributo que indica a getCor() do nó
    bool cor;  
    //! Atributo que lista os elementos do nó
    std::vector<NoRB<T>* > elementos;
 public:
    //! Construtor padrão, por padrão um nodo é negro
    ...
    //! Verifica a cor de um nodo. 
    //! Por padrão, todo nullptr é um nodo de cor negra.
    ...
    //! Retorna os elementos de um nodo
    ...
    //! Método para rebalencear os nodos filhos e pai de uma árvore
    ...
    //! Após feita a rotação, os filhos poderão ser recoloridos
    //! para manter as propriedades originais da árvore.
    NoRB<T>* rotate_right(NoRB<T> *arv) {
        NoRB<T> *nodo, *tmp;
    ...
    //! para manter as propriedades originais da árvore.
    NoRB<T>* double_rotate_right(NoRB<T>* arv) {
        arv->direita = rotate_left(arv->getDireita());
    ...
    //! da árvore. Após feita a rotação, os filhos poderão ser recoloridos
    //! para manter as propriedades originais da árvore.
    NoRB<T>* double_rotate_left(NoRB<T>* arv) {
        arv->esquerda = rotate_right(arv->getEsquerda());
        return rotate_left(arv);
    }

    //! Método para fazer uma inserção simples em uma árvore
    //! binária. Verificamos se o dado é maior ou menor e
    //! vamos caminhando pela árvore até achar um nodo condizente 
    //! e com o devido ponteiro vazio. Depois da inserção feita, 
    //! colorimos o novo nodo filho e ajustamos os ponteiros.
    ...
    //! Método para inserir um novo nodo na nossa árvore RB.
    ...
    //! Método para verificar se após a inserção, a árvore
    ...
    //! Procuramos o dado a ser deletado na árvore. Quando acharmos,
    ...
    //! Método para encontrar o nodo com menor valor de dado na árvore.
    ...
    //! Método que retorna o dado guardado por um nodo.
    T* getDado() {
        return dado;
    }

    //! Método para encontrar se um certo dado existe
    ...
    //! Método para colocar os nodos da árvore na lista "elementos" em preOrdem
    ...
    //! Método para colocar os nodos da árvore na lista "elementos" em emOrdem
    ...
    //! Método para colocar os nodos da árvore na lista "elementos" em posOrdem
